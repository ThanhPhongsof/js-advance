window.addEventListener("load", function () {
  ...
  const date = "Thu Jun 30 2022 21:48:46 GMT+0700 (Indochina Time)";
  function handlerCountdonw(date) {
    const endDate = new Date(date).getTime();
    let currentDate = new Date();
    if (isNaN(endDate) || endDate - currentDate.getTime <= 0) return;
    setInterval(() => {
      handlerCalculate();
    }, 1000);
